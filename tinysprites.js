...
(function(){
  ...
  const hexToRgba=hex=>{const v=parseInt(hex.replace('#',''),16);return[(v>>8&15)*17,(v>>4&15)*17,(v&15)*17,255];};
 ...
 function create(w,h,fillIndex,paletteHex){
  ...
  function fromImageData(img){
    ...
    const pal=[[0,0,0,0]]; const map={"0,0,0,0":0};
    const out=new Uint8Array(w*h); let p=0;
    for(let i=0;i<data.length;i+=4){
      const a=data[i+3];
      if(a===0){ out[p++]=0; continue; }
      const key=data[i]+","+data[i+1]+","+data[i+2]+","+a;
      let idx=map[key];
      if(idx==null){ idx=pal.length; if(idx>26) idx=0; map[key]=idx; pal.push([data[i],data[i+1],data[i+2],a]); }
      out[p++]=idx;
    }
  ...
  function b128decode(str){
    ...
    for(let i=0;i<str.length;i++){
      acc=(acc<<7)|str.charCodeAt(i); bits+=7;
  ...
  function decodePacked(str){
    ...
    let w=TinySprites.defaultW|0, h=TinySprites.defaultH|0;
    if(dimStr){
      if(dimStr.includes('x')){ const [ws,hs]=dimStr.split('x'); if(ws) w=p36(ws)|0; if(hs) h=p36(hs)|0; }
      else{ w=h=p36(dimStr)|0; }
    }
    const payload=parts[1]||'';
    if(!payload) return{w,h,data:new Uint8Array(w*h),palette:[[0,0,0,0]]};
    let pos=0; const count=parseInt(payload[pos++],36)|0;
    const palette=[[0,0,0,0]];
    for(let i=0;i<count;i++){ const hex=payload.slice(pos,pos+3); pos+=3; palette.push(hexToRgba(hex)); }
    let order=''; if(payload[pos]==='Z'){ order='Z'; pos++; }
    const data128=payload.slice(pos);
    const totalBytes=(w*h+1>>1);
    const comp=b128decode(data128);
    const xor=[]; for(let i=0;i<comp.length;i++){ const b=comp[i]; if(b===0&&i+1<comp.length){ const c=comp[++i]; for(let k=0;k<c;k++) xor.push(0); } else xor.push(b); }
    const packed=new Uint8Array(totalBytes); let prev=0;
    for(let i=0;i<totalBytes;i++){ const val=(xor[i]^prev)&255; packed[i]=val; prev=val; }
    const data=new Uint8Array(w*h); let di=0;
    for(let i=0;i<packed.length;i++){ const byte=packed[i]; data[di++]=byte>>4; if(di<w*h) data[di++]=byte&15; }
    if(order==='Z'){ const m=zigzagMap(w,h); const out=new Uint8Array(data.length); for(let j=0;j<m.length;j++) out[m[j]]=data[j]; return{w,h,data:out,palette}; }
